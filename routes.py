...
def check_filters(restaurant, filters):
    for filter in filters:
        if filter == 'high_rating' and restaurant.rating < 4.5:
            ...
            return False
        elif filter == 'outdoor_seating' and not check_outdoor_seating(
                restaurant.place_id):
            return False
        elif filter == 'bbq_grill' and not check_bbq_grill(
                restaurant.place_id):
            return False
    return True
...
def check_outdoor_seating(place_id):
    ...
    if details_response['status'] == 'OK':
        ...
        return any('outdoor seating' in review.get('text', '').lower()
...
def check_bbq_grill(place_id):
    ...
    if details_response['status'] == 'OK':
        reviews = details_response['result'].get('reviews', [])
        return any('bbq grill' in review.get('text', '').lower()
                   for review in reviews)
    return False
...
def favorites():
    return render_template('favorites.html',
                           favorites=current_user.favorite_restaurants.all())
